defmodule Bonfire.UI.Coordination.TestDrivenCoordination do
  ...
  def handle_cast({:suite_started, _opts}, config) do
    debug(config, "Tests started, with config:")

    {:noreply, config}
  end

  def handle_cast({:module_finished, %{tests: tests} = tested_module}, config) do
    # debug(tested_module, "Test for module done")
  ...
  def handle_test(%{state: {:invalid, module}} = test, _config),
    do: post_test(test, "The test seems invalid (#{inspect(module)})", ["Test fails"])

  # FIXME, skipped/excluded tests are not included in :module_finished
  ...
  def handle_test(%{state: {:failed, failures}} = test, config) do
    error =
      ExUnit.Formatter.format_test_failure(test, failures, 1, 90000, &formatter(&1, &2, config))
      |> String.split(["\n"])
      |> Enum.drop(2)
      |> Enum.join("\n")
